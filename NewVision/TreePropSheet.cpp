...
// ---------------------------------------------------------------------
void CTreePropSheet::OnBnClickedButtonOk() {
	GetActivePage()->UpdateData(true);
	AfxGetApp()->m_pMainWnd->GetMenu()->CheckMenuItem(ID_SETTINGS_PARAMETERS, MF_UNCHECKED | MF_BYCOMMAND);
	m_wndToolBar.GetToolBarCtrl().CheckButton(ID_SETTINGS_PARAMETERS, false);
	ShowWindow(SW_HIDE);
}
// ---------------------------------------------------------------------
void CTreePropSheet::OnBnClickedButtonCancel() {
...
// ---------------------------------------------------------------------
void CTreePropSheet::OnBnClickedButtonApply() {
//	((Model*)GetActivePage())->doc->SetModifiedFlag();
	GetActivePage()->UpdateData(true);
}
// ---------------------------------------------------------------------
BOOL CTreePropSheet::PreTranslateMessage(MSG* pMsg) 
{
   if (pMsg->message == WM_KEYDOWN && pMsg->message >= WM_KEYFIRST && pMsg->message <= WM_KEYLAST)
   {
	     BOOL handled = TRUE;
...
BOOL CTreePropSheet::OnInitDialog() 
{
	...
	// Resize the sheet at the bottom to make space for the buttons
	int width = 80, height = 25, offset = 6;
	rectSheet.bottom += 2*offset;
    MoveWindow(rectSheet);

	// Add OK, Cancel and Apply buttons at the bottom
	ScreenToClient(rectSheet);
	CRect r_ok(rectSheet.right - 3*width - 4*offset, rectSheet.bottom- height - offset, rectSheet.right - 2*width - 4*offset, rectSheet.bottom - offset);
	CRect r_cancel(rectSheet.right - 2*width - 3*offset, rectSheet.bottom- height - offset, rectSheet.right - 1*width - 3*offset, rectSheet.bottom - offset);
	CRect r_apply(rectSheet.right - 1*width - 2*offset, rectSheet.bottom- height - offset, rectSheet.right - 0*width - 2*offset, rectSheet.bottom - offset);
	// ------ Uncomment below to use as a realy property sheet ---------------
	m_ok.Create("Ok",	BS_PUSHBUTTON|WS_CHILD|WS_VISIBLE|WS_TABSTOP, r_ok, this, IDC_BUTTON_OK);
	m_cancel.Create("Cancel",	BS_PUSHBUTTON|WS_CHILD|WS_VISIBLE|WS_TABSTOP, r_cancel, this, IDC_BUTTON_CANCEL);
	m_ok.SetFont(GetFont());
	m_cancel.SetFont(GetFont());
	m_apply.Create("Apply",	BS_PUSHBUTTON|WS_CHILD|WS_VISIBLE|WS_TABSTOP, r_apply, this, IDC_BUTTON_APPLY);
	GetActivePage()->SetDefID(IDC_BUTTON_APPLY);

...
#define STRING_QFE		"VERSION_QFE"
#define STRING_DATE		"VERSION_BUILD_DATE"
#define STRING_TIME		"VERSION_BUILD_TIME"

bool	bIncrementVersion;
bool	bSimulate;
bool	bVerbose;
char*	sVersionFile;

char* str_Delete( char* buf, int index, int count /*=1*/ )
{
	char* c;
	assert(buf);
	assert(index>=0);
...
bool str_ReplaceOnce( char* buf, const char* find, const char* repl )
{
	...
	if( insert )
	{
		...
		{
			int len;
			for( len = strlen(pos); len>=flen; len-- )
...
bool GetDate(char* str_date, char* str_time, bool q)
{
	...
	if(str_time)
		sprintf(str_time, q?"\"%02u:%02u:%02uUTC\"":"%02u/%02u/%02uUTC", st.wHour, st.wMinute, st.wSecond);
...
bool CheckCommandLine(int argc, char* argv[])
{
	...
	for( i=1; i<argc; i++)
	{
		if( argv[i][0]=='/' || argv[i][0]=='-' )
		{
			...
			if( !strcmp(argv[i]+1, "simulate") || !strcmp(argv[i]+1, "sim"))
				bSimulate = true;
...
{
	printf(	"Usage: %s [-h] [-inc] [-sim] [-v] [version.ver]", argv[0] );
...
bool WriteVersionFile(HANDLE hFile, const char *str)
{
	...
	if( FALSE == WriteFile(hFile, str, strlen(str), &dwBytesWritten, NULL) || dwBytesWritten != strlen(str))
	{
		return false;
	}

...
//            IVideoWindow* pVideoWindow;
//            nextFilter->QueryInterface(IID_IVideoWindow, (void**)&pVideoWindow);

			HRESULT hr = _cvcamCapGraphBuilder->FindInterface(NULL, &MEDIATYPE_Video, nextFilter,
				IID_IVideoWindow, (void **)&_cvcamVideoWindow2);
			if(hr != NOERROR)
				return hr;  
            ...
            _cvcamVideoWindow2->put_Owner((OAHWND)hWnd);

            long flags;
            _cvcamVideoWindow2->get_WindowStyle(&flags);
	        _cvcamVideoWindow2->put_WindowStyle(flags & (~WS_CAPTION) | WS_CHILD);
	        _cvcamVideoWindow2->put_MessageDrain((OAHWND)hWnd);
            ...
            _cvcamVideoWindow2->SetWindowPosition( 0, 0, rndwidth, rndheight );
		    const char* name = cvGetWindowName(hWnd);
		    cvResizeWindow(name, rndwidth, rndheight);

...
void CameraModelXML::internalInit()
{
	double sa;
	double ca;
	double sb;
...
void CameraModelXML::setGeometry(int width, int height, double ncx, double nfx, double dx, double dy, double dpx, double dpy)
{
	mImgWidth = width;
	mImgHeight = height;
	mNcx = ncx;
...
void CameraModelXML::setIntrinsic(double focal, double kappa1, double cx, double cy, double sx)
{
	mFocal = focal;
	mKappa1 = kappa1;
	mCx = cx;
...
void CameraModelXML::setExtrinsic(double tx, double ty, double tz, double rx, double ry, double rz)
{
	mTx = tx;
	mTy = ty;
	mTz = tz;
...
bool CameraModelXML::fromXml(std::istream& is)
{
...
void CameraModelXML::toXml(std::ostream& os) const
{
...
bool CameraModelXML::fromDat(std::istream& is, int width, int height)
{
	is >> mNcx;
	is >> mNfx;
	is >> mDx;
...
CvPoint3D32f CameraModelXML::coordsImage2Real(CvPoint2D32f p, double z)
{
	//bool CameraModelXML::imageToWorld(double Xi, double Yi, double Zw, double& Xw, double& Yw)
	CvPoint3D32f p3;
	double Xd;
	double Yd;
	double Xu;
...
void CameraModelXML::distortedToUndistortedSensorCoord (double Xd, double Yd, double& Xu, double& Yu)
{
...
CvPoint2D32f CameraModelXML::coordsReal2Image(CvPoint3D32f p, double pRw) {
	//bool CameraModelXML::worldToImage(double Xw, double Yw, double Zw, double& Xi, double& Yi)
	CvPoint2D32f p2;
	double xc;
	double yc;
	double zc;
	...
	if (isInit)
	{
		...
		xc = mR11 * p.x + mR12 * p.y + mR13 * p.z + mTx;
		yc = mR21 * p.x + mR22 * p.y + mR23 * p.z + mTy;
		zc = mR31 * p.x + mR32 * p.y + mR33 * p.z + mTz;
...
void CameraModelXML::undistortedToDistortedSensorCoord (double Xu, double Yu, double& Xd, double& Yd)
{
	double Ru;
	double Rd;
	double lambda;
...
bool CameraModelXML::undistortedToDistortedImageCoord (double Xfu, double Yfu, double& Xfd, double& Yfd)
{
	bool done = false;
	double Xu;
	double Yu;
...
bool CameraModelXML::distortedToUndistortedImageCoord (double Xfd, double Yfd, double& Xfu, double& Yfu)
{
	bool done = false;
	double Xd;
	double Yd;
...
bool CameraModelXML::worldToCameraCoord (double xw, double yw, double zw, double& xc, double& yc, double& zc)
{
	bool done = false;
...
bool CameraModelXML::cameraToWorldCoord (double xc, double yc, double zc, double& xw, double& yw, double& zw)
{
	bool done = false;
	double common_denominator;
